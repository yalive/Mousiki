...
object TimeUtils {
    ...
    fun getTotalSongsDuration(list: List<Song>): Long {
        return if (list.isEmpty()) 0 else list.map { it.duration }
    ...
    fun formatElapsedTime(secs: Long, cxt: Context?): String {
        ...
        context?.let {
            ...
            when {
                hours > 0 -> sb.append(hours).append(hrStr).append(comma).append(minutes)
                    .append(minStr)
                    .append(comma).append(seconds).append(secStr).toString()
                minutes > 0 -> sb.append(minutes).append(minStr).append(comma).append(seconds)
                    .append(secStr).toString()
